...
export default function Settings() {
  ...
  return (
    <div>
      ...
      {/* ── Admin tools ─────────────────────────────────────────────── */}
      {user?.role === 'admin' && <VenueDefaultsCard />}
...
// -- Venue Defaults (admin only) --------------------------------------------
// Lets admins set:
//   • per-stage default capacity (used when a show has no per-show override)
//   • default day-sheet times (load-in, sound-check, doors, sets, curfew)
//   • optional per-day-of-week overrides that stomp the default for that day
//
// Individual show fields (doorsTime, showTime, capacity) still take priority
...
function VenueDefaultsCard() {
  ...
  // Sync local form state whenever the server-side venue defaults change.
  useEffect(() => {
    setStages(venue.stages || {})
    setDefaultRow(venue.daySheet?.default || {})
    setByDay(venue.daySheet?.byDay || {})
    // If any per-day overrides exist, auto-expand the day columns.
    if (Object.keys(venue.daySheet?.byDay || {}).length) setShowDayCols(true)
  }, [venue])

  function setStageCap(key, val) {
    const n = val === '' ? '' : Number(val)
    setStages(s => ({ ...s, [key]: { ...(s[key] || {}), capacity: n } }))
  }

  function setDefaultTime(itemKey, val) {
    setDefaultRow(d => ({ ...d, [itemKey]: val }))
  }
  ...
  function setDayTime(dow, itemKey, val) {
    setByDay(prev => {
      ...
      if (Object.keys(cur).length) next[dow] = cur
  ...
  async function handleSave() {
    ...
    try {
      ...
      for (const [k, v] of Object.entries(stages)) {
        ...
        if (Number.isFinite(cap) && cap > 0) stagesOut[k] = { capacity: cap }
      }
      await save({
        stages: stagesOut,
        daySheet: { default: defaultRow, byDay },
      })
  ...
  return (
    <div className="card">
      ...
      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 14 }}>
        Applied to every new show. A show’s own <code>capacity</code>, <code>doorsTime</code>,
        and <code>showTime</code> fields always win when they’re filled in.
      </div>

      {/* ── Stage capacities ────────────────────────────────────────── */}
      <div style={{ fontWeight: 600, marginBottom: 8 }}>Stage capacity</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 20 }}>
        {STAGE_KEYS.map(s => (
          <Field key={s.key} label={s.label} hint="Default max attendance for this stage.">
            <input
              className="input" type="number" min={1} step={1}
              value={stages[s.key]?.capacity ?? ''}
              onChange={e => setStageCap(s.key, e.target.value)}
            />
          </Field>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexWrap: 'wrap', gap: 8 }}>
        <div style={{ fontWeight: 600 }}>Day-sheet default times</div>
              ...
              <tr key={it.key} style={{ borderTop: '1px solid var(--border)' }}>
                ...
                {showDayCols && DOW_LABELS.map((_, i) => (
                  <td key={i} style={{ padding: '6px 8px' }}>
                    <input
                      className="input" type="time"
                      value={byDay[String(i)]?.[it.key] || ''}
                      placeholder={defaultRow[it.key] || ''}
